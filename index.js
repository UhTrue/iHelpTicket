...
const ownerID = '272501224002158603'; // Set your ID in here. Do it by copy and pasting it through discord.
...
client.on('ready', () => { 
    client.user.setGame('iHelp System | Message Me For Help');
...
client.on('message', async message => { 
    ...
    if (message.guild === null) {
        ...
        if (!active || !found) {
            ...
            active = {};
            let modRoles = guild.roles.find("name", "@Support Reader"); // Find the Mod/Admin roles so only Admin/Mods will see the tickets. Add it in the quotes
            let everyone = guild.roles.find("name","@" + "everyone");
            let bot = guild.roles.find("name","Bots");
            channel = await guild.createChannel(`${message.author.username}-${message.author.discriminator}`);
                channel.setParent(''); // Management Category ID
                channel.setTopic(`_complete to close the Ticket | Message for ${message.author.tag} | ID: ${message.author.id}`);
            ...
            const newChannel = new Discord.RichEmbed()
                ...
                .setFooter('iHelp Ticket Created')
        ...
        const dm = new Discord.RichEmbed()
            ...
            .setFooter(`Your message has been sent -- A iHelp Support member will be in contact soon.`);
    ...
    if (support) {
        ...
        if (message.content.toLowerCase() === "_complete") {
            const complete = new Discord.RichEmbed()
                ...
                .setDescription('*Your iHelp Ticket has been marked as **Complete**. If you wish to reopen this, or create a new one, please send a message to the bot.*');
...
client.login("NDUyNjUwNzYyNjk5NDA3MzYx.XOwaNA.b0Wsbigk6pnuFx8t_t296iEv0oI"); //Add the token to your bot user here
